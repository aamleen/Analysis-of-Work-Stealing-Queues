...
hclib_task_t *hpt_steal_task(hclib_worker_state *ws) {
    ...
    while (pl != NULL) {
        ...
        for (int i=1; i<nb_deq; i++) {
            ...
            if(d==NULL)
                printf("NULL returned while stealing <--delete from line 59 src/hclib-hpt.c\n");
...
inline void init_hc_deque_t(hc_deque_t *hcdeq, place_t *pl) {
    //int anchor[]={0,0,0};
    hcdeq->deque.anchor=0UL;
    //hcdeq->deque.data=(hclib_task_t *)malloc(INIT_DEQUE_CAPACITY*sizeof(deque_t));
    //hcdeq->deque.tail = 0;
    hcdeq->pl = pl;
    hcdeq->ws = NULL;
    hcdeq->nnext = NULL;
