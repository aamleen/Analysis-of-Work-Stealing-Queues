...
int chunkSize = 8;    // ber of nodes to move to/from shared area
...
void genChildren(Node * parent, int wid) {
	...
	int Children, childType;
	StealStack *ss = threadStealStacks[wid];

	ss->maxTreeDepth = max(ss->maxTreeDepth, parent->height);

	Children = uts_numChildren(parent);
	childType   = uts_childType(parent);

	// record ber of children in parent
	parent->numChildren = Children;

	// construct children and push onto stack
	if (Children > 0) {
		int i, j;
		int localSize = 2*chunkSize;
		int stack_size = Children + ss->stack_head - ss->stack_tail;
		...
		for (i = 0; i < Children; i++) {
			...
			for (j = 0; j < computeGranularity; j++) {
				...
				// computeGranularity controls ber of rng_spawn calls per node
			...
			if (ss->localWork > localSize) {
				if ((ss->nNodes % polling_interval) == 0) {
					...
					ss->localWork -= chunkSize;
					hclib::async([=]() {	// can be shipped to remote node as well
 ...
 * 
 *   Note: tree size is measured by the ber of
 *         push operations
 */
...
void showStats(double walltime) {
	...
	uts_showStats(nthreads, elapsedSecs, tnodes, tleaves, mheight);
}
